...
SELECT 
	COUNT (DISTINCT X.id_intento) AS #Clientas, CAST(ROUND(AVG(item_resp),2) AS DEC(10,2)) promedio 
FROM RESPUESTA R
	INNER JOIN INTENTO X ON R.id_intento = X.id_intento
	INNER JOIN ENCUESTA E ON X.id_encuesta = E.id_encuesta 
	INNER JOIN USUARIO U ON X.id_usuario = U.id_usuario           
...
FROM RESPUESTA R
	...
	INNER JOIN ENCUESTA E ON X.id_encuesta = E.id_encuesta 
	INNER JOIN USUARIO U ON X.id_usuario = U.id_usuario
	INNER JOIN RADICA Z ON U.id_usuario = Z.id_usuario
	INNER JOIN ZONA A ON Z.id_zona = A.id_zona
...
FROM RESPUESTA R
	INNER JOIN INTENTO X ON R.id_intento = X.id_intento
	INNER JOIN ENCUESTA E ON X.id_encuesta = E.id_encuesta 
	INNER JOIN LINEA L ON E.id_linea  = L.id_linea 
WHERE MONTH(fecha_inte) IN (12)
...
FROM intento X
	INNER JOIN USUARIO U ON X.id_usuario = U.id_usuario 
	INNER JOIN ENCUESTA E ON X.id_encuesta = E.id_encuesta
	INNER JOIN RESPUESTA R ON R.id_intento = X.id_intento
...
FROM RESPUESTA R
	...
	INNER JOIN ENCUESTA E ON X.id_encuesta = E.id_encuesta 
	INNER JOIN USUARIO U ON X.id_usuario = U.id_usuario 
	INNER JOIN ZONA Z ON X.id_usuario = U.id_usuario 
	INNER JOIN LINEA L ON E.id_linea = L.id_linea 
...
FROM
	(SELECT 
		 R.id_intento, R.id_pregunta, R.item_resp, E.id_linea, U.id_usuario, 
	...
	INNER JOIN INTENTO X ON R.id_intento = X.id_intento
	INNER JOIN ENCUESTA E ON X.id_encuesta = E.id_encuesta 
	INNER JOIN USUARIO U ON X.id_usuario = U.id_usuario) T
GROUP BY Trimestre
